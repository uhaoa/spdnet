...
int main(int argc, char *argv[]) {
    ...
    // http
    connector.async_connect(spdnet::net::end_point::ipv4("127.0.0.1", atoi(argv[1])),
                            [](std::shared_ptr<http_session> session) {
                                session->set_http_response_callback(
                                        [](const http_response &response, std::shared_ptr<http_session> session) {
                                            session->shutdown();
                                        });
    ...
    connector.async_connect(spdnet::net::end_point::ipv4("127.0.0.1", atoi(argv[1])),
                            [](std::shared_ptr<http_session> session) {
                                session->set_ws_handshake_success_callback([session]() {
                                    ...
                                    websocket_frame frame1("hi websocket server.", ws_opcode::op_text_frame, true,
                                                           true);
                                ...
                                session->set_ws_frame_enter_callback(
                                        [](const websocket_frame &frame_data, std::shared_ptr<http_session> session) {
                                            std::cout << "recv ws frame . opcode:" << (uint32_t) frame_data.get_opcode()
                                                      << "payload content:" << frame_data.get_payload() << std::endl;
                                        });
