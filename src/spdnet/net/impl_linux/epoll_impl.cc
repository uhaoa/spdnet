    ...
    namespace net {
        namespace detail{
                ...
                void EpollImpl::closeSession(TcpSession::Ptr session)
                {
                    ...
                    delay_tasks.emplace_back([&loop , session_ptr](){
                        loop.removeTcpSession(session_ptr->sock_fd());
                        // Avoid circular reference
                        session_ptr->socket_impl_data().channel_->session_.reset();
                    });

                    struct epoll_event ev{0, {nullptr}};
                    ::epoll_ctl(loop.getImpl().epoll_fd(), EPOLL_CTL_DEL, session_ptr->sock_fd(), &ev);
                    session->onClose();
                }
